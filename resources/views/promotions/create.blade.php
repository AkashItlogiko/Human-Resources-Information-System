...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="mb-4">
        <div>Current Salary: <strong>৳ <span id="old_salary_display">{{ number_format($employee->salary,2) }}</span></strong></div>
    </div>

    <form action="{{ route('promotions.store', $employee) }}" method="POST" id="promoForm">
        @csrf
        <input type="hidden" id="old_salary" value="{{ $employee->salary }}">

        <div class="mb-3">
            <label class="block mb-1">Increase Type</label>
        ...
        <div class="mb-3">
            <label class="block mb-1">Value (amount or percent)</label>
            <input type="number" step="0.01" min="0" name="increment_value" id="increment_value" class="w-full p-2 border rounded" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="effective_date" class="w-full p-2 border rounded">
        </div>
